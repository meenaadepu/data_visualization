...
#df=pd.read_excel(r"D:\Meena\Automation\plots\inputs\2228\GPIO18_AE\18AE_IPU_IPD_Spec.xlsx")
print(df.shape)
...
color_dict = dict({'TT': 'coral','Design Spec':'red'})
markers = {'TT': 'o','Design Spec': "X"}
...
for col in fcols:
        ...
        for k, ele in enumerate(df_VDDIO_names):
            ...
            for j in df_VDDC_names:
                ...
                if not df_filter.empty:
                   ...
                   #     print("mm1")
                   #     color_dict = dict({'FF': '#2980B9', 'FS': 'green', 'SF': 'violet', 'SS': 'orange', 'TT': 'coral','Min Design Spec': "red", 'Max Design Spec': "red"})
                   #     markers = {'FF': 'o', 'FS': 'o', 'SF': 'o', 'SS': 'o', 'TT': 'o', 'Min Design Spec': "X",'Max Design Spec': "X"}
                   ...
                   #     g = sns.pointplot(data=df_filter, x='name', y=col, marker='x')
                   # else:
                   #     print("mm")
                   #     g = sns.scatterplot(x='name', y=col, hue='Process', s=65, edgecolor='black', style="Process", markers=markers, palette=color_dict, data=df_filter, legend='full')

                   color_dict = dict({'FF': '#2980B9', 'FS': 'green', 'SF': 'violet', 'SS': 'orange', 'TT': 'coral','Min Design Spec': "red", 'Max Design Spec': "red",'Design Spec':'red'})
                   markers = {'FF': 'o', 'FS': 'o', 'SF': 'o', 'SS': 'o', 'TT': 'o', 'Min Design Spec': "X",'Max Design Spec': "X",'Design Spec':'X'}
                   g = sns.scatterplot(x='name', y=col, hue='Process', s=65, edgecolor='black', style="Process", markers=markers, palette=color_dict, data=df_filter, legend='full')
                   df1 = df_filter[df_filter["Process"] == "Design Spec"]
                   ref.extend(list(df1[col]))
                 #  print(df_filter.head(3))
                 #   df_filter2 = df_filter[df_filter['Process']=='Design Spec']
                 #   g = sns.pointplot( x='name',hue='Process', y=col,style="Process",palette=color_dict, data=df_filter2, linestyles=['--'],legend='full')
    ...
    if (col == 'VOH_DRV_01' or col == 'VOH_DRV_10' or col =='VOH_DRV_11' or  col == 'VIL_SMT_1' or col == 'VIL_SMT_0'):
   ...
   # setting temp separaters ticks position#
    ...
    if (col == 'IIL(nA)' or col == 'IIH(nA)'or col == 'IPD(uA)' or col == 'IPU(uA)'):
        ax1.set_ylabel(col, size=12.5, fontdict=dict(weight='bold'), labelpad=10)
    else:
