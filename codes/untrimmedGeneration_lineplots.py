...
folder = "untrimmed"
inputpath = r"D:\Meena\Automation\ATV\2227\tsvs\untrimmed"
...
for path, subdirs, files in os.walk(inputpath):
    for name in files:
        ...
        for i in sheets:
            df = pd.read_excel(fname,sheet_name=i, header=[1,2])
           # print(df.columns)
            df.columns = df.columns.map('_'.join)
            df.rename_axis('temperatue').reset_index()
            df1 = pd.concat([(df.iloc[:, 2:3]), (df.iloc[:, 49:])], axis=1)
...
for l,j in enumerate(cols_total):
    sns.set(style="ticks")
    Title1 = "Untrimmmed TS Accuracy Measurement"
...
#ax2.set_xticks(np.arange(-40, 130, 5))
